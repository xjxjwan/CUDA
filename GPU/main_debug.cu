...
// function: data recording
void dataRecordDebug(Grid uHost, Grid u, const int case_id, const double nCellsX, const double nCellsY,
    const double x0, const double y0, const double dx, const double dy, std::string name) {
    ...
    // data recording
    std::ostringstream oss;
    oss << caseFolder << "/GPU_Case" << case_id << "_" << name << ".txt";
    std::string fileName = oss.str();
    std::fstream outFile(fileName, std::ios::out);
...
// function: data recording
void fluxRecordDebug(Grid fluxHost, Grid flux, const int case_id, const double nCellsX, const double nCellsY,
    const double x0, const double y0, const double dx, const double dy, std::string name) {

    // copy data from GPU to CPU
    cudaMemcpy(fluxHost.data, flux.data, (nCellsX + 2 * nGhost - 1) * (nCellsY + 2 * nGhost - 1) * NUM_VARS * sizeof(double),
        cudaMemcpyDeviceToHost);
    ...
    double* flux_ij = new double[NUM_VARS];
    for (int i = nGhost; i < nCellsX + nGhost; i++) {
        for (int j = nGhost; j < nCellsY + nGhost; j++) {
            for (int v = 0; v < NUM_VARS; v++) {
                flux_ij[v] = fluxHost(i, j, v);
            }
            outFile << x0 + (i - nGhost + 0.5) * dx << ", " << y0 + (j - nGhost + 0.5) * dy
            << ", " << flux_ij[0] << ", " << flux_ij[1] << ", " << flux_ij[2] << ", " << flux_ij[3]
            << std::endl;
        }
    }
    outFile.close();
...
int main() {
    ...
    // Grid fluxHost(nCellsX - 1, nCellsY - 1, CPU);  // flux on CPU for debugging
